...
plt.figure('Americio') #plot per vedere i dati
...
ds=np.sqrt(np.sum(data)) #errore poissoniano. Forse ho sbagliato la formula?
...
popt,pcov = curve_fit(gaus,x,data,p0=[1,93,7]) #trova i parametri ottimali (popt) e la matrice di covarianza(pcov).I parametri iniziali li ho stimati ad occhio
...
print('il fattore moltiplicativo è %.3f, la media è %.3f, la sigma è %.3f' % (a,x0,sig))
